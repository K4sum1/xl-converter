...
def test_b2sum_valid_path():
    ...
    with patch.object(Path, "open", mock_open(read_data=file_content)) as mock_file:
        assert utils.b2sum("path/to/file") == expected_hash, "The hash does not match."

def test_b2sum_invalid_path():
    with patch.object(Path, "open", side_effect=OSError("File not found")) as mock_file:
        with pytest.raises(OSError):
            utils.b2sum("path/to/file")

def test_b2sum_invalid_digest_size():
    with patch.object(Path, "open", mock_open(read_data=b"test")) as mock_file:
