...
def test_config_location_linux_portable():
    with patch("data.constants.platform.system", return_value="Linux"),        patch("data.constants.ConfigManager.getboolean", return_value=True) as mock_getboolean,        patch("data.constants.isRunningInFlatpak", return_value=False):
        reload(constants)
...
def test_config_location_linux_default():
    with patch("data.constants.platform.system", return_value="Linux"),        patch("data.constants.ConfigManager.getboolean", return_value=False) as mock_getboolean,        patch("data.constants.isRunningInFlatpak", return_value=False):
        reload(constants)
        assert constants.CONFIG_LOCATION == os.path.expanduser('~/.config/xl-converter')

def test_config_location_linux_flatpak():
    with patch("data.constants.platform.system", return_value="Linux"),        patch("data.constants.ConfigManager.getboolean", return_value=False) as mock_getboolean,        patch("data.constants.isRunningInFlatpak", return_value=True),        patch("data.constants.os.environ.get", return_value="/tmp/path"):
        reload(constants)
...
def test_config_location_win_portable():
    with patch("data.constants.platform.system", return_value="Windows"),        patch("data.constants.ConfigManager.getboolean", return_value=True) as mock_getboolean,        patch("data.constants.isRunningInFlatpak", return_value=False):
        reload(constants)
        assert constants.CONFIG_LOCATION == os.path.join(constants.PROGRAM_FOLDER, "user_data")
        mock_getboolean.assert_called_once_with("General", "portable_user_data", False)

def test_config_location_win_default():
    with patch("data.constants.platform.system", return_value="Windows"),        patch("data.constants.ConfigManager.getboolean", return_value=False) as mock_getboolean,        patch("data.constants.isRunningInFlatpak", return_value=False):
        reload(constants)
