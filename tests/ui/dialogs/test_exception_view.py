...
def test_addItem(exception_view):
    ...
    with patch("ui.dialogs.exception_view.QTreeWidgetItem", return_value=mock_item),        patch.object(exception_view.exceptions_t, "addTopLevelItem") as mock_addTopLevelItem:
...
def test_saveToFile_happy_path(exception_view):
    ...
    with patch.object(exception_view, "isEmpty", return_value=False) as mock_isEmpty,        patch("ui.dialogs.exception_view.QFileDialog.getSaveFileUrl", return_value=(mock_dlg, None)) as mock_getSaveFileUrl,        patch.object(exception_view, "_writeCsv") as mock__writeCsv,        patch("ui.dialogs.exception_view.VERSION", "version") as version:
...
def test_saveToFile_empty(exception_view):
    with patch.object(exception_view, "isEmpty", return_value=True),        patch.object(exception_view.notifications, "notify") as mock_notify,        patch.object(exception_view, "_writeCsv") as mock__writeCsv:
        exception_view.saveToFile()
...
def test_saveToFile_dlg_invalid(exception_view):
    ...
    with patch.object(exception_view, "isEmpty", return_value=False),        patch("ui.dialogs.exception_view.QFileDialog.getSaveFileUrl", return_value=(mock_dlg, None)),        patch.object(exception_view, "_writeCsv") as mock__writeCsv:
        exception_view.saveToFile()
...
def test__writeCsv_happy_path(exception_view):
    ...
    with patch("builtins.open", mock_open()) as mock_csv_file,        patch("ui.dialogs.exception_view.csv.writer") as mock_csv_writer,        patch.object(exception_view.notifications, "notifyDetailed") as mock_notifyDetailed:
...
def test__writeCsv_sad_path(exception_view):
    ...
    with patch("builtins.open", side_effect=OSError()) as mock_csv_file,        patch("ui.dialogs.exception_view.csv.writer") as mock_csv_writer,        patch.object(exception_view.notifications, "notifyDetailed") as mock_notifyDetailed:
        exception_view._writeCsv(file_path, rows)
...
def test_reset(exception_view):
    with patch.object(exception_view, "close") as mock_close,        patch.object(exception_view, "clear") as mock_clear:
