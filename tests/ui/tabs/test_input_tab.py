...
def test_addFiles_flatpak_has_permission(input_tab_patched):
    ...
    with patch("ui.tabs.input_tab.FLATPAK", True) as mock_flatpak,      patch("ui.tabs.input_tab.Path.__str__", return_value="/tmp/user/Pictures/image.jpg") as mock_path_str:
        input_tab.addFiles()
        input_tab.notify.notify.assert_not_called()
...
def test_addFiles_flatpak_no_permissions(input_tab_patched):
    ...
    with patch("ui.tabs.input_tab.FLATPAK", True) as mock_flatpak,      patch("ui.tabs.input_tab.Path.__str__", return_value="/run/user/1000/doc/123456789/Pictures/image.jpg") as mock_path_str:
        input_tab.addFiles()
